...
class BECSimulation:
    ...
    def apply_losses_and_heating(self, N, T, dt):
        ...
        # Two-body and three-body loss model
        n = N / (4/3 * np.pi * np.prod(np.sqrt(2*k*T / (self.m * np.array([self.omega_x, self.omega_y, self.omega_z])**2))))
        K2 = 1e-16  # Two-body loss coefficient (m^3/s)
        K3 = 4e-29  # Three-body loss coefficient (m^6/s)
        loss_rate = K2 * n + K3 * n**2
        survival_probability = np.exp(-loss_rate * dt)
        survivors = np.random.random(N) < survival_probability
        self.positions = self.positions[survivors]
        self.velocities = self.velocities[survivors]
        N_new = len(self.positions)
        
        # Three-body recombination heating
        E_heat = 0.1 * K3 * n**2 * N * dt  # Adjust the factor as needed
        
        # Avoid division by zero when calculating dT
        if N_new > 0:
            dT = 2 * E_heat / (3 * N_new * k)
        else:
            dT = 0
    ...
    def system_evolution(self, t, state):
        ...
        if stage == 'MOTLoading':
            ...
            gamma_heat, N = self.apply_losses_and_heating(N, T, dt)
