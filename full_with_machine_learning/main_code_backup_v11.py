...
def mot_loading_and_compression_and_gray_molasses(atoms, trap, P_y, P_z, B_z):
    ...
    try:
        # 99 ms of MOT loading and initial compression
        for _ in range(int(0.099 / dt)):
            atoms.velocities *= 0.9905  # Increased cooling rate
            atoms.update(dt)
        ...
        logging.error(f"Error in mot_loading_and_compression_and_gray_molasses: {e}")
        raise
...
def run_full_sequence(params):
    ...
    try:
        ...
        atoms = mot_loading_and_compression_and_gray_molasses(atoms, trap, P_y_interp(time_array[:int(0.1/dt)]), 
                                            P_z_interp(time_array[:int(0.1/dt)]), B_z_interp(time_array[:int(0.1/dt)]))
