...
class BECSimulation:
    ...
    # Precompute stage boundaries for faster lookup
    ...
    def interpolate_params(self, t):
        for i, (start, end, name) in enumerate(self.stage_boundaries):
            if start <= t < end:
                ...
                # If this is the last stage, use its parameters without interpolation
        ...
        # If t is beyond the last stage, return the parameters of the last stage
        return self.stage_params[self.stages[-1]['name']]

    def calculate_trap_depth(self, P, w):
        """Calculate the peak trap depth for a given power and waist."""
        return 4 * P / (np.pi * w**2)
    
    def calculate_trap_frequency(self, U, w):
        """Calculate the radial trap frequency for a given trap depth and waist."""
        return np.sqrt(4 * U / (self.m * w**2)) / (2 * np.pi)


    def trap_frequencies(self, P_y, P_z, t):
        print("P_y", P_y)
        print("P_z", P_z)
        # Calculate trap depths
        U_y = self.calculate_trap_depth(P_y, self.params['wx'])
        print("U_y", U_y)
        U_z = self.calculate_trap_depth(P_z, self.params['wy'])
        print("U_z", U_z)

        # Calculate radial frequencies
        nu_y = self.calculate_trap_frequency(U_y, self.params['wx'])
        nu_z = self.calculate_trap_frequency(U_z, self.params['wy'])

        # Calculate axial frequency (approximation for crossed trap)
    ...
    def psd(self, N, T, nu_x, nu_y, nu_z):
        ...
        n_cp = N * (self.m * omega_mean**2 / (2 * np.pi * k * T))**(3/2)
    ...
    def raman_cooling_rate(self, T, P_R, P_p, B_z, delta):
        ...
        # B_z is already in Gauss, so we use it directly
        # 1 Gauss = 1e-4 Tesla, and mu_B is in J/T, so we need to adjust the conversion
        detuning = 2 * np.pi * 4.33e9 + (self.mu_B * 1e-4 * B_z) / hbar
    
        # Calculate the effective Rabi frequency
        Omega_eff = Omega_R**2 / (2 * detuning)
        
        # Calculate the effective scattering rate
    ...
    def heating_rate(self, N, T, nu_x, nu_y, nu_z):
        nu_mean = np.mean([nu_x, nu_y, nu_z])
        return h * nu_mean * self.collision_rate(N, T, nu_x, nu_y, nu_z) / (3 * k)  # Reduced heating

    def light_assisted_loss_rate(self, N, T, nu_x, nu_y, nu_z, P_p):
        nu_mean = np.mean([nu_x, nu_y, nu_z])
        n0 = N * (self.m * nu_mean**2 / (2 * np.pi * k * T))**(3/2)
        beta = 5e-15 * P_p  # Adjusted coefficient based on optical pumping power
        return beta * n0 * (T / (1e-6))**0.5

    def evaporation_rate(self, N, T, nu_x, nu_y, nu_z, P_y, P_z, t):
    ...
    def system_evolution(self, t, state):
        ...
        nu_x, nu_y, nu_z = self.trap_frequencies(P_y, P_z, t)
        print("nu_x: ", nu_x, "nu_y: ", nu_y, "nu_z: ", nu_z)
        ...
        if stage == 'MOTLoading':
            ...
            gamma_cool = self.raman_cooling_rate(T, P_R, P_p, B_z, delta)  # B_z is used directly in Gauss
            gamma_heat = self.heating_rate(N, T, nu_x, nu_y, nu_z)
            dNdt = -self.params['gamma_bg'] * N - self.light_assisted_loss_rate(N, T, nu_x, nu_y, nu_z, P_p)
            dTdt = -gamma_cool * T + gamma_heat
        elif stage.startswith('Evap'):
            gamma_evap = self.evaporation_rate(N, T, nu_x, nu_y, nu_z, P_y, P_z, t)
    ...
    def run_simulation(self):
        ...
        for t, N, T in zip(sol.t, sol.y[0], sol.y[1]):
            stage = next((name for start, end, name in self.stage_boundaries if start <= t <= end), self.stages[-1]['name'])
            params = self.interpolate_params(t)
            nu_x, nu_y, nu_z = self.trap_frequencies(params['P_y'], params['P_z'], t)
...
class BECVisualizer:
    ...
    def generate_frame(self, result, grid_size=200):
        ...
        nu_x, nu_y, nu_z = self.sim.trap_frequencies(result['P_y'], result['P_z'], result['time'])
